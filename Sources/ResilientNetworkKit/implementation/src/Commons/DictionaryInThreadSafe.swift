...
public final class DictionaryInThreadSafe<Key: Hashable & Sendable, Value: Sendable>: @unchecked Sendable {
  // MARK: Lifecycle
  ...
  /// Returns the keys of the dictionary in a thread-safe manner.
  public var keys: [Key] {
    concurrentQueue.sync {
      Array(threadUnsafeDict.keys)
    }
  }
  ...
  /// Removes all key-value pairs from the dictionary in a thread-safe manner.
  public func removeAll() {
    concurrentQueue.async(flags: .barrier) {
      self.threadUnsafeDict.removeAll()
    }
  }
  ...
  private let concurrentQueue = DispatchQueue(
    label: "com.CommonSharedKit.DictionaryInThreadSafe",
